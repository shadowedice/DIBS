...
class Holidays:
        ...
        try:
            ...
            for msg in self.messages:
                ...
                if msg.channel.id == ctx.message.channel.id and itemCount == count:
                    ...
                    user = self.database.GetField("Christmas", ["ServerID", "UserID"], [ctx.message.server.id, ctx.message.author.id], ["Bag", "Gift", "Coal", "TotalBags"])
                    bags = user[0]
                    gifts = user[1]
                    coal = user[2]
                    totalBags = user[3]
    ...
    async def christmasScore(self, ctx):
        text = ""
        users = self.database.GetFields("Christmas", ["ServerID"], [ctx.message.server.id], ["UserID", "Bag", "Gift", "Coal"])
