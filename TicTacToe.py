...
class TicTacToe:
            ...
            if state is not None:
                ...
                if numArgs == 1:
                    ...
                    if text == "board":
                        ...
                        await self.bot.say("It is currently " + playerturn + "'s turn.")
                    elif text == "quit":
                        if ctx.message.author == state.player1 or ctx.message.author == state.player2:
                            ...
                            await self.bot.say("Only " + state.player1.name + " or " + 
                                                state.player2.name + "can end the game.")
                    else:
                        await self.bot.say("Unknown command " + text + ".")
                            ...
                            if row >= 1 and row <= 3 and col >= 1 and col <=3:
                                if state.board[row-1][col-1] == '*':
                                    ...
                                    if check is not None:
                                        await self.bot.say(check)
                                        del self.game_states[ctx.message.channel]
                                else:
                                    await self.bot.say("That space is already taken!")
                            else:
                                await self.bot.say("Invalid row or column!")
                        except ValueError:
                            await self.bot.say("Invalid row or column!")
    ...
    #overload function for options      
    #@commands.command(pass_context=True)    
    #async def ttt(self, ctx, text : str):
    #    state = self.game_states.get(ctx.message.channel)
    #    if state is not None:
    #        if text == "board":
    #            await self.bot.say(self.printBoard(state.board))
